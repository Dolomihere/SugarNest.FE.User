      ...
      <main className="w-full px-4 py-8 mx-auto space-y-6 max-w-7xl">
        ...
        <div className="p-6 bg-white border rounded-lg shadow-md">
          ...
          <section className="p-6 mb-6 bg-white border border-gray-200 shadow-md rounded-xl">
            <h3 className="mb-2 font-semibold text-md text-brown-700">Sở thích</h3>
            <div className="flex flex-wrap gap-2">
              {[
                ...
                <span key={i} className="px-3 py-1 text-xs transition-colors rounded-full bg-amber-100 text-amber-700 hover:bg-amber-200">{tag}</span>
              ))}
            </div>
          </section>
          ...
          <section className="grid gap-6 md:grid-cols-3">
            ...
            <aside className="space-y-6">
              <nav className="p-4 space-y-3 text-sm bg-white shadow rounded-xl">
                {[
                  ...
                  <button key={btn.key} onClick={() => { setActiveTab(btn.key); setViewOrderId(null); setEditProfile(false); }}
            ...
            <section className="space-y-6 md:col-span-2">
              {activeTab === 'profile' && !editProfile && <ProfileTab user={user} />}
              {activeTab === 'orders' &&
                (viewOrderId
    ...
    <div className="p-6 space-y-4 bg-white shadow-md rounded-xl">
      ...
      <div className="grid gap-4 text-sm text-gray-700 sm:grid-cols-2">
        {[
          ...
          <p key={i}>
            <FontAwesomeIcon icon={field.icon} className="mr-2 text-amber-600" />
      ...
      <div className="pt-6">
        <h3 className="mb-2 text-lg font-semibold text-brown-700">Lịch sử đăng nhập gần đây</h3>
...
function OrderHistoryTab({ onViewDetail }) {
  ...
  return (
    <div className="p-6 space-y-6 bg-white shadow-md rounded-xl">
      ...
      {orders.map(o => (
        <div key={o.id} className="p-4 transition-shadow border rounded-lg bg-amber-50 hover:shadow-md">
          <div className="flex items-center justify-between">
            <div>
              <p><strong>Mã đơn:</strong> {o.id}</p>
              <p><strong>Ngày:</strong> {o.date}</p>
              <p><strong>Trạng thái:</strong> {o.status}</p>
              <p className="font-semibold text-amber-700"><strong>Tổng:</strong> {o.total.toLocaleString()}đ</p>
            </div>
            <button
          ...
          <div className="mt-3">
            <h4 className="mb-1 text-sm font-semibold text-brown-600">Lịch sử giao dịch</h4>
      ...
      <div>
        ...
        <div className="space-y-4">
          {order.items.map((i, idx) => (
            <div key={idx} className="flex items-center gap-4 p-2 transition-colors rounded-lg hover:bg-gray-100">
              <img src={i.image} alt={i.name} className="object-cover w-16 h-16 border rounded" />
...
function CouponTab() {
  ...
  return (
    <div className="p-6 space-y-4 bg-white shadow-md rounded-xl">
      ...
      {coupons.map((c, idx) => (
        <div key={idx} className="p-4 text-sm transition-shadow border rounded-lg border-amber-200 bg-amber-50 hover:shadow-md">
          <p><strong>Mã:</strong> <span className="font-bold text-amber-700">{c.code}</span></p>
